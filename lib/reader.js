...
ExcelReader.prototype = Object.create(null, {
  ...
  _readCellStyles: {
    value: function* _readCellStyles() {
      // Read the cell styles so we can identify dates.
      ...
      yield this._parseXML('xl/styles.xml', function(e) {
        ...
        if (inCellXfs && e.tag === 'xf' && e.type === 'start') {
          var n = parseInt(e.attrs.numFmtId);
          styles.push({
            numFmtId: n,
            type: (n >= 14 && n <= 17) ? 'd' : 'n'
          });
      ...
      while (i <= end) {
        ...
        if (value !== undefined) {
          ...
          if (type === 's') {
            ...
            // It has a style which will be an index into the global styles
            // array.  See if we can infer the type from the style.
            var s = this.workbook.styles[parseInt(e.attrs.s)];
            if (s && s.type === 'd') {
