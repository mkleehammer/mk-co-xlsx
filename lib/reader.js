...
// var XMLParser = require('../../mk-co-xml/index.js').Parser;
...
Sheet.prototype = Object.create(null, {
  ...
  read: {
    value: function* read() {
      ...
      while (true) { // loop over rows to support skipEmptyRows
        var x = yield* this._skipTo('row');
        if (!x)
          return null;

        var index = parseInt(x.attributes.r);
        var cols = [];
        ...
        while (true) { // loop over columns
          ...
          if (x.type === 'elementStart' && x.tag === 'c') {
            yield this._readCell(cols, x);
          }
        }

        if (cols.length > 0 || !this.workbook.skipEmptyRows) {
          return {
            index: index,
            cols: cols
          };
  ...
  _readCell: {
    value: function* _readCell(cols, x) {
      ...
      if (!v || v.tag !== 'v') {
        // There was no value tag, which is an empty cell.  We're going to
        // ignore the cell (leave it undefined) so that we don't add empty
        // values to the end of cells.  Sometimes if you delete data from a
        // bunch of rows, they stay in the spreadsheet due to formatting.
        return;
      }
